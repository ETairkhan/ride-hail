...
func (rs *RidesService) CancelRide(rideID string, reason string, passengerID string) (dto.RidesCancelResponseDto, error) {
	log := rs.mylog.Action("CancelRide").With("ride_id", rideID, "passenger_id", passengerID)
	ctx, cancel := context.WithTimeout(rs.ctx, time.Second*10)
	defer cancel()

	// First, get the ride to check if it exists and belongs to the passenger
	ride, err := rs.RidesRepo.GetRideByID(ctx, rideID)
	if err != nil {
		log.Error("Failed to get ride", err)
		return dto.RidesCancelResponseDto{}, fmt.Errorf("ride not found")
	}

	// Verify the ride belongs to the passenger
	if ride.PassengerId != passengerID {
		log.Warn("Passenger attempted to cancel another passenger's ride")
		return dto.RidesCancelResponseDto{}, fmt.Errorf("access denied")
	}

	// Check if ride can be cancelled
	if !rs.canRideBeCancelled(ride.Status) {
		log.Warn("Ride cannot be cancelled in current status", "current_status", ride.Status)
		return dto.RidesCancelResponseDto{}, fmt.Errorf("ride cannot be cancelled in status: %s", ride.Status)
	}

	// Cancel the ride
	err = rs.RidesRepo.CancelRide(ctx, rideID, reason, passengerID)
	if err != nil {
		log.Error("Failed to cancel ride", err)
		return dto.RidesCancelResponseDto{}, err
	}

	// TODO: Notify driver via WebSocket if driver was assigned
	// TODO: Send cancellation event to message broker

	log.Info("Ride cancelled successfully", "reason", reason)

	response := dto.RidesCancelResponseDto{
		RideID:      rideID,
		Status:      "CANCELLED",
...
func (rs *RidesService) canRideBeCancelled(status string) bool {
