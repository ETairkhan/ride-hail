...
func NewDriverService(db *pgx.Conn, rabbitMQ *rabbitmq.RabbitMQ, wsHub *websocket.Hub, cfg *config.Config) *DriverService {
	ds := &DriverService{
		db:       db,
		rabbitMQ: rabbitMQ,
		wsHub:    wsHub,
...
func (ds *DriverService) GoOnline(ctx context.Context, driverID string, lat, lng float64) (*types.OnlineResponse, error) {
...
func (ds *DriverService) GoOffline(ctx context.Context, driverID string) (*types.OfflineResponse, error) {
	tx, err := ds.db.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to begin transaction: %w", err)
	}
...
func (ds *DriverService) UpdateLocation(ctx context.Context, driverID string, update models.LocationUpdate) (*models.Coordinates, error) {
	tx, err := ds.db.Begin(ctx)
	if err != nil {
		return nil, fmt.Errorf("failed to begin transaction: %w", err)
	}
