...
import (
	...
	rideWebsocket "ride-hail/internal/adapter/websocket"
	"ride-hail/internal/common/config"
	"ride-hail/internal/common/types"
	"ride-hail/internal/domain/models"
)

type DriverHandler struct {
	driverService types.DriverService
}
...
func NewDriverHandler(driverService types.DriverService) *DriverHandler {
...
func (s *Server) Start() error {
	addr := fmt.Sprintf(":%s", s.cfg.ServicesConfig.DriverLocationServicePort)
...
func (s *Server) serveDriverWebSocket(w http.ResponseWriter, r *http.Request) {
	// Extract driverID from URL path properly
	path := r.URL.Path
	driverID := strings.TrimPrefix(path, "/ws/drivers/")
...
func (h *DriverHandler) StartRide(w http.ResponseWriter, r *http.Request) {
	...
	response := types.StartRideResponse{
...
func (h *DriverHandler) CompleteRide(w http.ResponseWriter, r *http.Request) {
	...
	response := types.CompleteRideResponse{
		RideID:         result["ride_id"].(string),
		Status:         result["status"].(string),
...
// Helper function to extract driver ID from URL path
func extractDriverID(path string) string {
	// Remove "/drivers/" prefix and any trailing paths
	driverID := strings.TrimPrefix(path, "/drivers/")
	// Remove any trailing paths after the driver ID
	if idx := strings.Index(driverID, "/"); idx != -1 {
		driverID = driverID[:idx]
	}
	return driverID
}
